...
public class TerrainGeneratorController : MonoBehaviour
{
    ...
    {
        GameObject newTerrain = Instantiate(terrainTemplates[Random.Range
            (0, terrainTemplates.Count)].gameObject, transform);
    ...
    {
        return gameCamera.ViewportToWorldPoint(
            new Vector2(0f, 0f)).x + areaStartOffset;
    }private float GetHorizontalPositionEnd()
    {
        return gameCamera.ViewportToWorldPoint(
            new Vector2(1f, 0f)).x + areaEndOffset;
    }
    // pool function
    private GameObject GenerateFromPool(GameObject item, Transform parent)
    {
        ...
        {
            // if item list not defind, create new one
            pool.Add(item.name, new List<GameObject>());
        }

        // crate new one if no item available in pool
